...
def I_j(t,T_open):
    ...
    if t < T_0 or t > T_0+T_open:
        ...
        return C*A*np.sqrt(2*(P_inlet-P_t[min(int(100*t), len(P_t)-1)])/rho)

# 求解P_t,将求解出的p_t存入P_t中
def P(t,T_open):
    P_t.append(P_0+sum([E_poly3(P_t[min(int(100*j), len(P_t)-1)],p)/rho_P(P_t[min(int(100*j), len(P_t)-1)])/V*(I_j(j,T_open)-E_j(j)) for j in np.arange(0, t, 0.01)]))
    return P_t[min(int(100*t), len(P_t)-1)]
...
for delt_t in np.arange(0, 0.2, 0.01):
    if delt_t==0:
        continue
    P_t=[P_0] # 油管随时间变化的压力
    t=0.01
    ...
    while P_t[-1] < 150:
        print('t:',t,'P_t:',P_t[-1])
        P(round(t,2),T_open_100+delt_t)
        if P_t[-1]== P_t[-2]:
            while round(t,2) % 100 != 0:
                P_t.append(P_t[-1])
                t+=0.01
        t+=0.01
        
    t_n.append((round(t,2), delt_t))

# 将 t_n 列表写入 CSV 文件
with open('result\\t_n_data_0.001.csv', 'w', newline='',encoding='UTF-8') as csvfile:
